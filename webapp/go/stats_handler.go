...
func getUserStatisticsHandler(c echo.Context) error {
	...
	if err := tx.GetContext(ctx, &viewersCount, "SELECT COUNT(*) FROM livestream_viewers_history h INNER JOIN livestreams l ON l.id = h.livestream_id AND l.user_id = ?", user.ID); err != nil && !errors.Is(err, sql.ErrNoRows) {
